...
import { Upload, X, FileText, CheckCircle2, Calendar, Sparkles } from 'lucide-react';
...
export const UploadMaterialModal: React.FC<UploadMaterialModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (defaultExamType) setRecommendedExam(defaultExamType);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalRecommendedExam =
      recommendedExam === 'Custom' ? customExamInput.trim() || 'General' : recommendedExam;
    ...
    onUpload({
      ...
      contentSnippet:
        contentSnippet ||
        `# ${title}\n\nKey academic lecture notes and chapter summaries for ${unit}. Recommended for ${finalRecommendedExam}.`,
    });
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto animate-in fade-in">
      <div className="bg-[#FEFEFA] border border-[#E5E4E2] rounded-2xl max-w-xl w-full p-6 md:p-8 shadow-2xl relative my-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Recommended Exam Selection (Timetable Integration) */}
          <div className="bg-[#F4F1EA]/70 border border-[#E5E4E2] rounded-xl p-3.5 space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider text-[#56615a] bg-white border border-[#D1D5DB] px-2 py-0.5 rounded-full flex items-center gap-1 shadow-2xs">
                <Sparkles className="w-2.5 h-2.5 text-[#e5a93c]" /> Timetable Sync
              </span>
            </div>

            {/* Quick select chips from scheduled exams for this subject */}
            {subjectExams.length > 0 && (
              <div>
                <span className="text-[11px] text-[#737874] block mb-1.5 font-medium">
                  Scheduled exams for {selectedSubject?.code || 'this course'}:
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {subjectExams.map((ex) => {
                    const isSelected = recommendedExam === ex.examType;
                    return (
                      <button
                        key={ex.id}
                        type="button"
                        onClick={() => setRecommendedExam(ex.examType || 'IAT 1')}
                        className={`text-xs px-2.5 py-1 rounded-lg font-semibold transition-all flex items-center gap-1.5 border ${
                          isSelected
                      ...
                      >
                        ...
                        <span className="text-[10px] opacity-75 font-normal">({ex.date})</span>
                      </button>
                    );
                  })}
                </div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    if (e.target.value === 'Custom') {
                      setRecommendedExam('Custom');
                    } else {
                      setRecommendedExam(e.target.value);
                    }
                  }}
                  className="paper-input w-full p-2.5 rounded-lg text-xs font-semibold text-[#1b1c1c] bg-white"
                >
                  ...
                  <option value="Custom">Custom Assessment Name...</option>
                </select>
              </div>
              ...
              {recommendedExam === 'Custom' ? (
                <div>
                  <input
                    type="text"
                    required
                    placeholder="Enter exam name (e.g. Unit Test 2)"
                ...
                <div className="flex items-center text-[11px] text-[#56615a] leading-tight px-1 font-medium">
                  <span>
                    Will be recommended under <strong className="font-bold">{recommendedExam}</strong> in the Exam Timetable.
                  </span>
                </div>
              )}
            </div>
